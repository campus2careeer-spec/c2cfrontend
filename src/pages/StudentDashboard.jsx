...
.s-search input{width:100%;padding:.48rem 2rem .48rem 2.1rem;border:1.5px solid var(--border2);border-radius:99px;background:var(--surface2);font-family:'DM Sans',sans-serif;font-size:.82rem;color:var(--slate);outline:none;transition:.2s}
...
export default function StudentDashboard() {
  ...
  const [searchTab, setSearchTab] = useState(null); // "jobs" | "courses" | null
  ...
  const getSkillGapAdvice = (jobSkills) => {
    if (!jobSkills || !profile?.skills) return null;

    const userSkills = profile.skills.map(s => s.toLowerCase());
    const requiredSkills = jobSkills.split(',').map(s => s.trim());
    ...
    // Find a course that covers at least one missing skill
    const recommendedCourse = courses.find(course =>
      course.skills?.some(cs => missing.some(ms => cs.toLowerCase() === ms.toLowerCase()))
    );

    return { missing, recommendedCourse };
  };
  ...
  const handleSearch = (value) => {
    ...
    const courseKeywords = ["course", "learn", "tutorial", "python", "react", "javascript", "java", "sql", "ml", "machine learning", "deep learning", "data", "design", "figma", "docker", "cloud", "aws", "devops", "flutter", "android", "swift", "kotlin", "php", "laravel", "vue", "angular", "excel", "power bi", "tableau", "tensorflow", "pytorch"];
    const lv = value.toLowerCase();
    const looksLikeCourse = courseKeywords.some(k => k.includes(lv) || lv.includes(k));
    // Switch tab based on context
    if (looksLikeCourse) {
    ...
    <>
      ...
      <nav className="s-nav">
        ...
        <div className="s-search">
          ...
          <input placeholder="Search jobs, courses…" value={searchQuery} onChange={e => handleSearch(e.target.value)}
            onKeyDown={e => { if (e.key === "Escape") clearSearch(); }} />
          {searchQuery && (
            <button onClick={clearSearch} style={{ position: "absolute", right: ".7rem", top: "50%", transform: "translateY(-50%)", background: "none", border: "none", cursor: "pointer", color: "var(--muted)", fontSize: ".75rem", fontWeight: 700, padding: "0", lineHeight: 1 }}>✕</button>
          )}
        </div>
          ...
          <AnimatePresence mode="wait">
                    ...
                    : <div className="feed-grid">
                      {vacancies.map((v, i) => (
                        <motion.div key={v.id || i} className="vac-card"
                          ...
                          {/* Inside the vacancies.map loop */}
                          {(() => {
                            const gap = getSkillGapAdvice(v.skills);
                            if (!gap) return null;
                            ...
                            return (
                              <div style={{
                                ...
                                <div style={{ fontSize: '0.7rem', fontWeight: 800, color: '#b45309', marginBottom: '0.4rem', textTransform: 'uppercase' }}>
                                  ⚠️ Skill Gap Detected
                                </div>
                                <div style={{ fontSize: '0.76rem', color: 'var(--slate)', marginBottom: '0.5rem' }}>
                                  You're missing: {gap.missing.map((s, idx) => (
                                    <span key={idx} style={{ fontWeight: 700, color: 'var(--rose)' }}>{s}{idx < gap.missing.length - 1 ? ', ' : ''}</span>
                                  ))}
                                </div>

                                {gap.recommendedCourse ? (
                                  <button
                                    onClick={() => window.open(gap.recommendedCourse.link || "#", "_blank")}
                                    style={{
                                      width: '100%',
                                      padding: '0.5rem',
                                  ...
                                  >
                                    Learn these skills: {gap.recommendedCourse.title} →
                                  </button>
                                ) : (
                                  <div style={{ fontSize: '0.65rem', color: 'var(--muted)', fontStyle: 'italic' }}>
                                    Check the 'Courses' tab to find materials for these skills.
                                  </div>
                                )}
                              </div>
                            );
                ...
                <div className="page-sec">
                  <div className="sec-head">
                    <div>
                      ...
                      <span className="sec-sub">· {searchQuery
                        ? `${allJobs.filter(j => j.job?.toLowerCase().includes(searchQuery.toLowerCase()) || j.industry?.toLowerCase().includes(searchQuery.toLowerCase()) || j.skills?.toLowerCase().includes(searchQuery.toLowerCase())).length} results for "${searchQuery}"`
                        : `${allJobs.length} openings`}</span>
                    </div>
                    {searchQuery && (
                      <button className="sec-link" onClick={clearSearch}>✕ Clear Search</button>
                    )}
                  </div>
                    ...
                    {searchQuery && allJobs.filter(j =>
                      ...
                      (j.description || j.desc)?.toLowerCase().includes(searchQuery.toLowerCase())
                        ...
                        <div className="empty-block" style={{ gridColumn: "1 / -1" }}>
                          ...
                          <div className="empty-text">Try a different keyword, or <button className="sec-link" onClick={clearSearch}>view all jobs</button>.</div>
                        </div>
                      )}
                  </div>
                </div>
            ...
            {activeTab === "courses" && (
              <motion.div key="courses" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
                {(() => {
                  const filteredCourses = courses.filter(c => !searchQuery ||
                    ...
                    <>
                      <div className="sec-head" style={{ marginBottom: "1.1rem" }}>
                        <div>
                          ...
                          <span className="sec-sub">· {searchQuery ? `${filteredCourses.length} results for "${searchQuery}"` : `${courses.length} available`}</span>
                        </div>
                        {searchQuery && (
                          <button className="sec-link" onClick={clearSearch}>✕ Clear Search</button>
                        )}
                      ...
                      {filteredCourses.length === 0 ? (
                        <div className="empty-block">
                          ...
                          <div className="empty-text">Try a different keyword, or <button className="sec-link" onClick={clearSearch}>browse all courses</button>.</div>
                        </div>
                      ) : (
